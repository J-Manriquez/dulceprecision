...
class ProductRepository {
  ...
  Future<int> actualizarProducto(Producto producto) async {
    ...
    try {
      ...
      // Si ocurre un error, lo registramos y lanzamos una excepción
      CustomLogger().logError('Error al actualizar producto con id ${producto.idProducto}: $e');
  ...
  Future<List<Producto>> getAllProductos() async {
    final db = await DatabaseHelper().database; // Obtenemos la instancia de la base de datos
    try {
      final List<Map<String, dynamic>> maps = await db.query('Productos'); // Realizamos la consulta

      // Convertimos la lista de mapas a una lista de productos
      return List.generate(maps.length, (i) {
        return Producto.fromMap(maps[i]); // Creamos un objeto Producto a partir del mapa
      });
    } catch (e) {
      // Si ocurre un error, lo registramos y lanzamos una excepción
  ...
  Future<int> insertProducto(Producto producto) async {
    ...
    try {
      ...
      return await db.insert(
        'Productos', // Nombre de la tabla
        producto.toMap(), // Convertimos el producto a un mapa usando el método toMap()
